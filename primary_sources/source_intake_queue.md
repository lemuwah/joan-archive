# Source Intake Queue — Joane-Centric

> **How to read this:** 🟢 PROVEN = we have the primary. 🔵 PROBABLE = strong chain, one link missing. 🟡 UNVERIFIED = claim exists, image not yet pulled. 🔴 NULL = searched, not found OR proven compilation layer. ⚠️ SUSPENDED = citation exists but verbatim text not located / citation invalidated. NULL is data.
> **Rule:** No citation = does not exist. Need Box.Folder.Page + 600 DPI + direct quote.

### The One Record That Matters
Joane Greene's ONLY VERIFIED appearance: **24 March 1681/82, Home-Place Deed** — Joan named as wife and as beneficiary of a 30-shilling life annuity after John's death. ⚠️ "Signs with mark" and the clause "or to her mother if she survive" are both UNVERIFIED — neither appears in F.L. Greene 1894 or the Worth 1921 abstract (John alone signs). See `contradictions/mother-clause-wording.md`. Everything below is the envelope we build around that moment.

**⚠️ Note:** The May 1682 Pawtuxet deed previously listed here as the "hero document" has been **SUSPENDED**. Its citation ("Vol. VII pp. 177–178") is invalid — that volume covers 1770–1776 (Gaspee commission records). Bates (1918–19) does not mention this deed. See `research/pawtuxet_deed_investigation.md`.

---

## 1. HERO PRIORITY — The Deed Itself

- [ ] **1682 Home-Place Deed — Joan's ONLY VERIFIED Appearance** 
    - **Status:** 🟢 PROVEN (F.L. Greene 1894, p. 10; NK Land Evidence)
    - **What it is:** The only surviving verified English record of Joane. John Greene divides Quidnessett land among sons Daniel (120ac) and James (60ac). Joan named with 30-shilling life-estate.
    - **Why Joane:** She is the named surviving beneficiary of the annuity. ~~Without her consent, deed invalid under coverture.~~ (The abstract shows John alone as grantor — Joan is not a party.) ~~The "or to her mother if she survive" clause has no English legal parallel.~~ ⚠️ WORDING UNVERIFIED — not in F.L. Greene 1894 or the Worth 1921 abstract; see contradictions/mother-clause-wording.md.
    - **Need:** 600 DPI scan of original from NK Land Evidence Book 1. Full TEI transcription. Scribe ID.

- [ ] **⚠️ 1682 Pawtuxet Deed — SUSPENDED** 
    - **Previous Status:** 🟢 PROVEN — NOW ⚠️ SUSPENDED
    - **What was claimed:** May 19, 1682 deed, John & Joan Greene → William Carpenter, £10, witnesses Wickes & Gorton. Joan's "free & voluntary consent" and "her marke."
    - **Problem:** Citation "Vol. VII pp. 177–178" = 1773 Gaspee commission records, NOT a 1682 deed. Bates does not mention it. Verbatim text not found in ANY digitized source.
    - **Full investigation:** `research/pawtuxet_deed_investigation.md`
    - **Resolution path:** Physical examination of RI Land Evidences Vol. I manuscript (FamilySearch Film 564389); Peirce Manuscripts (films 22291–22292); Providence town deed books.

## 2. ENVELOPE AROUND JOANE — 1672-1685

These deeds prove the world that made her 1682 consent necessary.

- [ ] **1672 Anashuecot Deed — Fones Record pp.166-169**
    - **Status:** 🟢 PROVEN
    - **Quote:** "Anashuecot, chiefe Sachem and heire properly..."
    - **Why Joane:** 7 signers all recorded as "his marke" — even sachem squas. Proves scribal construct that may hide women. Direct comparator for Joane's gendered recording.
    - **Need:** High-res IIIF manifest, scribe analysis.

- [ ] **1674 Awawsunks — Sachim Squa — RI Land Evidences Vol I p.49**
    - **Status:** 🟢 PROVEN
    - **Why Joane:** Correctly recorded as "X her marke" — shows clerk *could* record female correctly. Why not always? Hostile Reviewer test.
    - **Need:** High-res + comparison chart vs 1672.

- [ ] **1679 Affidavit — "forty years and more..."**
    - **Status:** 🔵 PROBABLE
    - **Why Joane:** Anchors family arrival c.1642. Establishes timeline for Quidnessett occupation.
    - **Need:** Locate second copy, verbatim TEI.

- [ ] **1685 Deed — Primary not yet individually pulled**
    - **Status:** 🟡 UNVERIFIED — HIGH PRIORITY ACTION
    - **Need:** Pull original image from Land Evidences, not abstract.

- [ ] **1677 East Greenwich — 48 Grantees**
    - **Status:** 🔵 PROBABLE
    - **Why Joane:** Who got land around Quidnessett? Shows speculator pressure.
    - **Need:** Bartlett partial search completed. Need pages 56-58 text-diff task.

- [ ] **Fones Record — 3 Cocumscussoc Tenant Deeds for Robert Greene**
    - **Status:** 🟢 PROVEN — Expand
    - **What it is:** From 1659-1887 Fones Record (FamilySearch, images only)
    - **Why Joane:** Proves tenant occupation, not just owner grants. Explains consent law.
    - **Need:** Pull 3 deeds, pin to GIS near Cocumscussoc.

### 3. ENVELOPE EXPANSION — Online Resources You Added

> These are finding aids + images from FamilySearch Catalog. Use to locate originals.

- [ ] **1648-1696 RI Land Evidences Vol I — Abstracts & Index (Dougine / Worthington)**
    - **Source:** FamilySearch Catalog — index & images
    - **Use:** Finding aid for all deeds above. Do NOT cite abstracts as primary. Use to get Book.Page for original pull.

- [ ] **1648-1795 Land and Public Notary Records of RI**
    - **Source:** FamilySearch — index & images — Microfilm of RI State Archives MSS
    - **Use:** May contain unrecorded deeds. Check for any Pawtuxet entries.

- [ ] **1646-1851 Proceedings of the General Assembly**
    - **Source:** FamilySearch — index & images
    - **Use:** Search 1679-1685 for Pawtuxet ratifications.

- [ ] **1636-1936 Rhode Island Boundaries — John Hutchins Cady**
    - **Source:** FamilySearch — images only
    - **Use:** For /digitization/gis/ — Georeference Quidnessett bounds. Needed for IIIF + GIS layer.

- [ ] **Peirce Manuscripts 1686-1826 — RI Land Evidences Copies**
    - **Source:** FamilySearch — images only — Microfilm at RI Historical Society
    - **Use:** Scribe comparison. Does Peirce copy "his marke" differently? Law 3 test. Also: check for Pawtuxet deed.

- [ ] **1638-1644 Records of RI + 1731-1773 Agents Accounts + 1747-1750 Certificates + 1786-1803 Account Book**
    - **Source:** FamilySearch
    - **Use:** Envelope context + Negative Evidence search.

## 4. NULL IS DATA — Prove You Searched

- [ ] **Violet Kettelle Papers, Box 2; RIHS Mss 1210, folders 12-13**
    - **Status:** 🔴 NULL — Isolated compilation layer
    - **Why:** Bow-and-arrow mark claim originates here. We digitize it to prove it's isolated and NOT in primary deeds. Contamination control.

- [ ] **1908 Reunion Newspaper Cutting**
    - **Status:** 🔴 NULL — Floating legend syndrome
    - **Why:** 'Born Clarke' legend. Quarantine in /contamination/. Show source of myth vs. deed.

## 5. QUARANTINE — Do Not Cite as Primary

- [ ] **1700s Native Soil Manuscripts Index (Ancestry $) — index only**
- [ ] **1886-1930 Dept of Interior Decisions on Pensions and Bounty-Land Claims (Ancestry $)**
    - **Rule:** Per Contamination Law — index-only, paywalled derivatives = /contamination/. Use only as clue to find original Box.Folder.Page. Never PROVEN.

---

### How to Contribute
1. Pick one checkbox
2. Pull 600 DPI image + note FamilySearch Film # + RI State Archives Box.Folder.Page
3. Add direct quote + TEI transcription
4. Tag: How does this help Joane? (🟢🔵🟡🔴⚠️)
5. PR to /sources/ or add to negative-evidence-log.md
