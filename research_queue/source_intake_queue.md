# Source Intake Queue

**Purpose:** Track physical documents and sources that need to be acquired, requested, or visited in person. These are sources the archive knows about but has not yet been able to examine.

**Rule:** A source in this queue is a LEAD, not evidence. Nothing from this queue enters the verified facts until it has been read and tagged.

**Last updated:** 2026-09-07

---

## Priority 1: Physical Pull Targets

These require visiting an archive or requesting a scan.

| # | Source | Location | What We're Looking For | Priority | Status |
|---|---|---|---|---|---|
| 1 | RIHS Mss 461 — "will complaint" | Rhode Island Historical Society, Providence | Possible Joan reference in a legal complaint related to John Greene's will. May contain information about Joan's status after 1682. | **HIGHEST** | ⚫ NOT EXAMINED |
| 2 | FamilySearch DGS 008204949, Images 9–12 | Online (FamilySearch) but requires paleographic skill | The manuscript original of the March 1682 deed. Resolves all 8 suspended items (S-001–S-008 except S-007 and S-008). AI transcription done; human paleographic verification pending. | **HIGHEST** | 🟡 AI READ, HUMAN VERIFICATION PENDING |
| 3 | Jamestown Town Records | FamilySearch (Land Evidence Records 1680–1899) | 1685 Clark-to-Brinley deed referencing John Greene as "now deceased" — resolves C-2 death date contradiction. | HIGH | ⚫ NOT EXAMINED |
| 4 | NK Land Evidence Records 1686–1921 | FamilySearch | 1695/96 Wightman deed; Benjamin-to-James deed. Resolves Joan's timeline post-1682. | HIGH | ⚫ NOT EXAMINED |
| 5 | First Baptist Church, Providence | Physical archive (fragmentary) | 1640s–1680s membership rolls — possible church record for Joan if English (Model B). | HIGH | ⚫ NOT SEARCHED |

## Priority 2: Remote Requests

These can be requested by email or online.

| # | Source | Contact | What We're Looking For | Status |
|---|---|---|---|---|
| 1 | NK Town Clerk Land Evidence Volumes | Kenneth Carlson, RI State Archives (kcarlson@sos.ri.gov) | Custody confirmation for earliest volumes. Already contacted. | Awaiting response |
| 2 | Clarke 1903 full text | Internet Archive (already downloaded) | Searched Sep 2026 — zero hits for Joan of Quidnessett. Clarke covers Warwick Greenes only. | ✅ COMPLETED — NULL |

## Priority 3: Future Targets

| # | Source | What We're Looking For | Notes |
|---|---|---|---|
| 1 | UK National Archives, Kew | CO 1/XX Colonial Office series — Narragansett correspondence | Unindexed for individuals; long shot |
| 2 | Dutch National Archives, The Hague | WIC records re: Richard Smith / Opdyck trading post | John's connection to Smith makes this relevant |
| 3 | Irish parish records | Model C (Irish origin) — Y-DNA points to Irish Sea cluster | Requires knowing a parish to search |
| 4 | Massachusetts State Archives, Boston | Court Records 1637–1645 — pre-RI records for John Greene | Possible marriage or church record |

---

*A source in this queue is a direction, not a destination. Follow it. See where it leads.*

*Maintained under the Seven Laws of the Joan Archive.*
