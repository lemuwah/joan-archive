...
Every identified contamination — AI hallucination, fabricated source, algorithmic
auto-fill guess, or uncorroborated secondary/circular compilation — must be logged in
the AI Contamination Log (`theory/source_spine.md` §6) and/or `contamination_index.md`
with its specific vector and the reasoning for its rejection. Contamination is never
silently deleted or quietly corrected in place — it is held on record with the same
rigor as proven evidence, so any agent or reader can see not just what is true, but
