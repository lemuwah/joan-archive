# Proof Piece #8 — The Evidence So Far Through Tomaquag's Lens (Revised 2026-08-31)
## What Do We See When We Look at Joan With Narragansett-Published Context?

**Status:** 🟡 ANALYTICAL — no new primary evidence; alignment check
**Last revised:** 2026-08-31 — Model B anomaly recount per Hostile Review; evidence table updated

---

## Model Comparison (CORRECTED per Hostile Review Cycle #4)

### Model A (Joan was Narragansett)
Explains Joan's pattern with one premise: she was an Indigenous woman whose identity was encoded in English legal forms.

### Model B (Joan was an unknown English woman)
Requires **3–4 independent anomalies** to all coincidentally apply to one woman:

1. **Total absence from all English record systems** (unique among Quidnessett wives — consolidates: no maiden name, no parish, no family, no pre-1682 record, no alternative Joan in four colonies, deeper erasure than comparable neighbor wives)
2. **The matrilineal inheritance clause** ("or to her mother if she survive" — ⚠️ WORDING UNVERIFIED — not in F.L. Greene 1894 or the Worth 1921 abstract; see contradictions/mother-clause-wording.md; no English legal parallel found *for that wording*; consolidates: clause itself + Female Inheritance Window timing)
3. **The survival anomaly** (Greene homestead uniquely undestroyed during KPW — with caveat: possible survivorship bias)
4. **The pre-war succession coincidence** (Quidnessett sachem authority transferred 1668–1672; Joan appears 1682)

⚠️ **Previous version counted 7–8 anomalies.** Hostile Review correctly identified that several were restatements of the same underlying fact. The argument is stronger with an honest count of 3–4.

### Model C (Joan = Beggarly) → ELIMINATED

**Neither model is proven.** The archive documents what the colonial record says and doesn't say.

---

*[Full Tomaquag alignment analysis, evidence table, and methodology preserved from original — see git history for complete version]*

*Joan Archive — theory/proof_pieces/08_tomaquag_alignment.md*
*Maintained under the 5 Laws of the Joan Archive*
*Status: REVISED 2026-08-31 — Model B recount applied*
